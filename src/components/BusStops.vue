<script lang="ts" setup>
  ...
  const toggleSortStops = () => {
    isSortingStopsAsc.value = !isSortingStopsAsc.value;
  };

  const toggleSortTimes = () => {
    isSortingTimesAsc.value = !isSortingTimesAsc.value;
  };

  const sortedStopsByBusLine = computed(() => {
    return [...stopsByBusLine.value].sort((a, b) => {
      if (isSortingStopsAsc.value) {
  ...
  const stopsByBusLine = computed(() => {
    return store.getters['timetable/stopsByBusLine'](selectedBusLine.value);
  });

  const timesForStopInBusLine = computed(() => {
    if (selectedBusLine.value && selectedStop.value) {
      return store.getters['timetable/timesForStopInBusLine'](selectedBusLine.value, selectedStop.value);
    }
